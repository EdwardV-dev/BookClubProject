...
function AddBook() {
  ...
  const handleFirstNameChange = (event) => {
    setBook({
      ...book,
      author: { ...book.author, authorFirstName: event.target.value },
    });
  };
  ...
  const handleLastNameChange = (event) => {
    setBook({
      ...book,
      author: { ...book.author, authorLastName: event.target.value },
    });
  };
  ...
  const handleSubmit = async (event) => {
    ...
    if (
      ...
      book.yearPublished.toString().length === 0
    ) {
    ...
    let correctTitle =
      book.bookTitle.charAt(0).toUpperCase() + book.bookTitle.substring(1);
    let correctAuthorFirstName =
      book.author.authorFirstName.charAt(0).toUpperCase() +
      book.author.authorFirstName.substring(1);
    let correctAuthorLastName =
      book.author.authorLastName.charAt(0).toUpperCase() +
      book.author.authorLastName.substring(1);
    let correctCaseGenre =
      book.genre.charAt(0).toUpperCase() + book.genre.substring(1);

    getCorrections();

    const updatedBook = {
      ...book,
      bookTitle: correctTitle,
      genre: correctCaseGenre,
      author: {
        ...book.author,
      ...
      try {
        ...
        if (response.status === 201 || response.status === 500) {
          ...
          if (response.status === 201) {
            ...
            setErrors([
              "Unable to add book. The entered book is most likely already associated with your account",
            ]);
            //   history.push("/books");
          }
        } else {
